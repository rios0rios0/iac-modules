---
name: code-review
description: "Review pull requests and diffs in iac-modules — the reusable Terraform modules, stacks and Terragrunt runner images — against the rios0rios0/guide standards, with extra weight on module interface stability, state and secret handling, and provider version pinning. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `iac-modules`

`iac-modules` holds reusable Terraform modules (`azm_app_service`, `azm_container_registry`, `azm_storage_container`), pre-composed stacks for direct Terragrunt use, and the Terragrunt runner Dockerfiles published to GitHub Packages. Module inputs and outputs are a public interface consumed by live infrastructure.
...
## What matters most in `iac-modules`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **A variable or output change is a breaking change.** Renaming, removing, or re-typing anything in `variables.tf` or `outputs.tf` breaks every consumer's plan. It needs the three-place breaking-change flag and a MAJOR bump — and a deprecation path where one is possible.
- **Watch for silent destroy-and-recreate.** A change to a `name`, a location, or another force-new attribute replaces live infrastructure. Say so explicitly in the review; a storage account or registry recreated in production is data loss.
- **Secrets never become Terraform outputs or defaults.** Sensitive values are marked `sensitive = true`, and a credential in a variable default is a **Critical** finding — state files are frequently readable.
- **Provider and module versions are pinned.** An unpinned `required_providers` constraint makes every plan non-reproducible and can pull a breaking major on a quiet day.
- **`azm_` is Azure and `aws_` is AWS.** A new module follows that prefix convention and is self-contained: `main.tf`, `variables.tf`, `outputs.tf`.
- **Stacks compose modules, they do not duplicate them.** Logic copied from a module into `stacks/` will drift.
- **Every variable has a `description` and, where it is safe, a `type` and a default.** An undescribed variable becomes an undocumented interface.
- **`terraform fmt -recursive` runs from the repository root; `terraform validate` runs inside a module directory** — there are no root-level `.tf` files, so validating from the root proves nothing.
- **The runner Dockerfiles pin their Terraform, Terragrunt, and CLI versions** and are published on release; `entrypoint.sh` supports both directory and command invocation modes — keep both working.
- `scm_integration.ps1` runs with deployment credentials; parameters must be quoted and never interpolated from untrusted input.
...
There is no automated test suite. Verification is `terraform fmt`, `terraform validate` inside each touched module, and a `terraform plan` against a scratch subscription — with the plan output read for replacements before merging.
...
- The three modules looking structurally identical — that is the module contract.
- The absence of `make` targets; this repository uses the Terraform CLI directly.
